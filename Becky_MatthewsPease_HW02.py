...
epatterns = []
epatterns.append('([a-zA-Z0-9+._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9_-]+)')
epatterns.append('([A-Za-z.]+)\s\s?@\s\s?([A-Za-z\..]+)\.edu')
epatterns.append('[A-Z0-9\._%+-]+(?:\s*@\s*|\s*\[*\s*at\s*\]*\s*)+[A-Z0-9\.-]+(?:\s*\.\s*|\s*\[*\s*dot\s*\]*\s*)[a-z]{2,6}')
epatterns.append('^[A-Za-z.\-]+@([a-z0-9-]+(\.[a-z0-9-]+)*?\.[a-z]{2,6}|(\d{1,3}\.){3}\d{1,3})(:\d{4})?$')
...
ppatterns = []
ppatters.append('(\d{3})-(\d{3})-(\d{4})')
ppatters.append('('^([0-9]( |-)?)?(\(?[0-9]{3}\)?|[0-9]{3})( |-)?([0-9]{3}( |-)?[0-9]{4}|[a-zA-Z0-9]{7})$')')
ppatters.append('\+?\(?([0-9]{3})\)?[-.]?\(?([0-9]{3})\)?[-.]?\(?([0-9]{4})\)?')
ppatters.append('/^(?:(?:\(?(?:00|\+)([1-4]\d\d|[1-9]\d?)\)?)?[\-\.\ \\\/]?)?((?:\(?\d{1,}\)?[\-\.\ \\\/]?){0,})(?:[\-\.\ \\\/]?(?:#|ext\.?|extension|x)[\-\.\ \\\/]?(\d+))?$/i')
ppatters.append('[(](\d{3})[)]\s(\d{3})-(\d{4})')
ppatters.append('[(](\d{3})[)][\s-](\d{3})-(\d{4})')
ppatters.append('[\+1]\s(\d{3})\s(\d{3})[\s-](\d{4})')
...
if __name__ == '__main__':
    ...
    main('C:/Users/becky/Downloads/ContactFinder/ContactFinder/data/dev', 'C:/Users/becky/Downloads/ContactFinder/ContactFinder/data/devGOLD')
